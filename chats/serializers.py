...
class ChatMessageSerializer(serializers.ModelSerializer):
    ...
    def get_attachment(self, message):
        if message.attachment_code == 'FILE':
            file_attachment = FileAttachment.objects.filter(id=message.attachment_id).first()
            if not file_attachment:
                return None
            return {"file": FileAttachmentSerializer(file_attachment).data}
        if message.attachment_code == 'AUDIO':
            audio_attachment = AudioAttachments.objects.filter(id=message.attachment_id).first()
